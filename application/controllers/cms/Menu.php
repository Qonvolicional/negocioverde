...
class Menu extends CI_Controller {
	...
	public function __construct(){
		...
		if($this->session->userdata("login")){
			...
			$modulo_activo=explode("/",$this->router->routes[$uri[0]]);
			
			$this->session->set_userdata("modulo_active", $this->Modulo_model->getModuloActivo($uri[0])->id);
			$this->session->set_userdata("submodulo_active",$this->Modulo_model->getModuloActivo($uri[0])->id);
			$this->session->set_userdata("modulo_string", $uri[0]);
			//Fin navegacion
