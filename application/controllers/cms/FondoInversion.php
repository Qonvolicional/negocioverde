...
class FondoInversion extends CI_Controller {
	...
	public function index(){
		$data = array(
			"fondos"=>$this->GestorContenido_model->getFondos(),
		);
		$this->load->view("layouts/dashboard/header");
		$this->load->view("layouts/dashboard/sidebar", $this->moduloControlador);
		$this->load->view("dashboard/cms/fondo/lista", $data);
		$this->load->view("layouts/dashboard/footer");
	}

	
	public function agregar(){
		$jquery = array("jquery" => "fondoAgrega.js");
		$this->load->view("layouts/dashboard/header");
		$this->load->view("layouts/dashboard/sidebar", $this->moduloControlador);
		$this->load->view("dashboard/cms/fondo/agrega2");
	...
	public function almacenar(){
		...
		$url_imagen = 'Fondo_'.$nombre.'__'.$fecha_creacion;
	    ...
	    if(($_FILES['imagen']['size']>0)){
	        ...
	        if ( !$this->upload->do_upload('imagen'))
	        {
                ...
                $mensaje.="<li>No se cargo correctamnete el logo del fondo de inversión. <sub>".$error['error']."</sub>.</li>";
	        }
	        else
	        {
                ...
                $mensaje.="<li>El logo del fondo de inversión se guardo de manera exitosa.</li>";
	        }
	    }
	    if($this->GestorContenido_model->setFondo($info))
	    {
	    	$mensaje.="<li> La información general del fondo se guardo de manera exitosa. </li></ul>";
	    	$this->session->set_flashdata("error",$mensaje);
	    	redirect(base_url()."cms_fondos");
	    }else{
	    	$this->session->set_flashdata("error","No se pudo guardar la información.");
			redirect(base_url()."cms_fondo/agregar");
	    }
	...
	public function editar($id){
		$data = array(
			"fondo" => $this->GestorContenido_model->getFondo($id)
		);
		$jquery = array("jquery" => "fondoEditar.js");
		$this->load->view("layouts/dashboard/header");
		$this->load->view("layouts/dashboard/sidebar", $this->moduloControlador);
		$this->load->view("dashboard/cms/fondo/editar", $data);
		$this->load->view("layouts/dashboard/footer",$jquery);
	}	
	...
	public function actualizar(){
		...
		$archivo = $this->GestorContenido_model->getFondo($id);
		$mensaje = "<h5>Información sobre la acción realizada</h5><ul>";
		$info = array(
					'nombre' => $nombre,
					'descripcion' => $descripcion,
	    ...
	    if(($_FILES['imagen']['size']>0)){
	        ...
	        {
                $error = array('error' => $this->upload->display_errors());
                $mensaje.="<li>No se pudo actualizar con éxito el logo del fondo de inversión ".$nombre.". <sub>".$error['error']."</sub></li>";
	    ...
	    if($this->GestorContenido_model->updateFondo($info, $id)){
	    	$mensaje.="<li>La información del fondo fue actualizada con éxito. </li></ul>";
	    	$this->session->set_flashdata("error",$mensaje);
	    	redirect(base_url()."cms_fondos");
	    }else{
	    	$this->session->set_flashdata("error","Error al actualizar información.");
			redirect(base_url()."cms_fondo/".$id);
	    }
	}

	public function estado($id){
		$info = $this->GestorContenido_model->getEstadoFondo($id);
		$estado = ($info->estado)?0:1;
		$data = array('estado'=>$estado);
		$this->GestorContenido_model->updateFondo($data, $id);
		redirect(base_url()."cms_fondos");
	}
