...
class Servicio extends CI_Controller {
	...
	public function index(){
		$data = array(
			"servicios"=>$this->GestorContenido_model->getServicios(),
		);
		...
		$this->load->view("dashboard/servicio/lista", $data);
		$this->load->view("layouts/dashboard/footer", $jquery);
	}

	
	public function agregar(){
		$jquery = array("jquery" => "servicio.js");
		$this->load->view("layouts/dashboard/header");
		$this->load->view("layouts/dashboard/sidebar", $this->moduloControlador);
		$this->load->view("dashboard/servicio/agrega2");
		$this->load->view("layouts/dashboard/footer", $jquery);
	}

	public function almacenar(){
		date_default_timezone_set('America/Bogota');
		$this->form_validation->set_rules("nombre","Nombre de servicio","required|min_length[3]|max_length[100]|is_unique[servicio.nombre]");
		if($this->form_validation->run())
		{
		    ...
		    if($this->GestorContenido_model->setServicio($info))
		    {
	...
	public function editar($id){
		$data = array(
			"servicio" => $this->GestorContenido_model->getServicio($id),
		);
		$jquery = array("jquery" => "servicio.js");
		$this->load->view("layouts/dashboard/header");
		$this->load->view("layouts/dashboard/sidebar", $this->moduloControlador);
		$this->load->view("dashboard/servicio/editar", $data);
		$this->load->view("layouts/dashboard/footer",$jquery);
	}	
	...
	public function actualizar(){
		...
		$nombre = $this->input->post("nombre");
		$servicioActual = $this->GestorContenido_model->getServicio($id);

		if ($nombre == $servicioActual->nombre) {
			$is_unique = "";
		}else{
			$is_unique = "|is_unique[servicio.nombre]";

		}
		//$this->form_validation->set_rules("cms_tipo_publicacion_id","Tipo de publicación","required");
		$this->form_validation->set_rules("nombre","Nombre de servicio","required|min_length[3]|max_length[100]".$is_unique);
		if($this->form_validation->run()){
			$descripcion = $this->input->post("descripcion");
			$info = array(
				'nombre' => $nombre,
		    ...
		    if($this->GestorContenido_model->updateServicio($info, $id)){
		    	redirect(base_url().$this->session->userdata("modulo_string"));
		    }else{
	...
	public function estado($id){
		$info = $this->GestorContenido_model->getEstadoServicio($id);
		$estado = ($info->estado)?0:1;
		$data = array('estado'=>$estado);
		$this->GestorContenido_model->updateServicio($data, $id);
		redirect(base_url().$this->session->userdata("modulo_string"));
	}
	
	public function eliminar($id){
		$this->GestorContenido_model->deleteServicio($id);
		redirect(base_url().$this->session->userdata("modulo_string"));
	}
